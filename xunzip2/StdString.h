...
//Converter functions provided by NikeJustDoItOk
//Borrowed from HCE
//Added by angrycamel on 4-1-05
/*inline wstring Convert( string in ) 
{ 
     wchar_t buf[1024];
	 for (int i=0; i<1024; i++)
		 buf[i] = L'';
//	 wchar_t *buf=malloc(1024*sizeof(wchar_t));
//	 swprintf(buf, L"%hs", in.c_str()); 
	for ( int i=0; i<(int)in.length(); i++ )
		mbtowc(&buf[i],&in[i],MB_LEN_MAX); 
	 return buf;
} 
*/

inline string Convert( wstring in ) 
{ 
     char buf[1024]; 
//     sprintf(buf, "%ls", in.c_str()); 
	 for (int i=0; i<(int)in.length(); i++)
		wctomb(&buf[i],in[i]);
     return buf; 
}
...
inline LPCWSTR Convert( string in) 
{ 
	 wchar_t buf[1024];
	 for (int i=0;i<1024;i++)
		 buf[i] = L'';
	 mbstowcs(buf,in.data(),in.length());
	 return buf;
} 

/*std::wstring a2w(const std::string& s) 
{ 
	std::vector<wchar_t> v(s.size() + 1);
	wchar_t *dest = &v[0];
	size_t nchars = mbstowcs(dest, s.c_str(), s.size() + 1);
	return (nchars == -1) ? std::wstring() : dest; 
}
*/
